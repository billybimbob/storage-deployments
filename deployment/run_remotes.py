...
def get_ips(file_name: str, only_data: bool=False):
    """
    ...
    global found_ips
    found_ips = 0
    def run_ip(ip: str, is_data: bool=True):
        valid = valid_ip(ip)
        global found_ips
...
def run_ips(ips: List[str], user: str, in_cmd: str, write_out: bool):
    ssh_stdout: List[str] = []
    for ip in ips:
...
if __name__ == "__main__":
    ...
    parse.add_argument("-c", "--cmd", 
        default='ls',
        help="command to run in each ip, cannot include double quote (\") "
            "characters")

    parse.add_argument("-d", "--data",
        action="store_true", help="apply to only data ips")
    ...
    parse.add_argument("-s", "--ssh",
        help="single target to run the command on, has precedence over "
            "the file arg")
    ...
    if ips != []:
        run_ips(ips, args.user, args.cmd, args.out)
